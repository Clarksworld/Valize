package com.zugavalize.app.widget.webview;
...
//                    fragment.getActivity().startActivity(intent);
    ...
    public boolean hasConnectivity(String loadUrl, boolean showDialog) {
        ...
        if ((info == null || !info.isConnected() || !info.isAvailable())) {
            ...
            if (showDialog){
                if (code.NO_CONNECTION_PAGE.length() > 0 && code.NO_CONNECTION_PAGE.startsWith("file:///android_asset")) {
                    browser.loadUrl(code.NO_CONNECTION_PAGE);
                } else {
    ...
    public static boolean urlShouldOpenExternally(String url){
         ...
         */
        if (code.OPEN_ALL_OUTSIDE_EXCEPT.length > 0) {
            for (String pattern : code.OPEN_ALL_OUTSIDE_EXCEPT) {
         ...
         */
        for (String pattern : code.OPEN_OUTSIDE_WEBVIEW){
            if (url.contains(pattern))
                return true;
        }
